...
| `WISPER_EXEC_TIMEOUT_MS` | `60000` | Client timeout (ms) for exec and release calls. For the streaming agent exec it is an inter-chunk **idle** window, *not* a wall-clock cap on total run time (the per-dispatch deadline bounds that). Unset or invalid → default. |
